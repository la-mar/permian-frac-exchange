...
import src.version
from src.settings import DATABASE_URI, OPERATOR_INDEX_SOURCE, OPERATORPATH
from src.stringprocessor import StringProcessor as sp
from src.util import chained, classproperty
...
if source == 'sql':
    from src.tables import Operator as Operator_Table
    _source_is_sql = True
else:
    Operator_Table = None
...
__release__ = src.version.__release__
...
class OperatorIndex(pd.DataFrame):
    ...
    def save(cls, result: pd.Series) -> None:
        ...
        result = result.to_frame().T[['operator', 'operator_alias', 'confidence', 'method']]
    ...
    def lookup(self, target: str) -> pd.Series:
        ...
        try:
            ...
            logger.error(f'Error looking up operator name ({e}) -- \n    Operator Name: {target}')
    ...
    def _fuzzy_match(self, target: str, scorer = None, score_cutoff = 85, limit = 1) -> pd.Series:
        ...
        # result = pd.Series(name = target)
        extracted: list = process.extractBests(target,
                                self.operator,
                                scorer = scorer,
                                limit = limit,
                                score_cutoff = score_cutoff
                                )
        df = pd.DataFrame.from_records(extracted, columns = ['name', 'index_score', 'fuzzy_score'])
        return df
    ...
    def refresh(self):
        ...
        if not new.empty: #TODO: this is clunky. need to fix later
            ...
            for idx, values in new.iterrows():
                try:
                    self.loc[idx] # try to lookup the index. Insert record if the lookup fails.
                except KeyError:
                    self.loc[idx] = values
...
class FileIndex(OperatorIndex):
    ...
    def save(self) -> None:
        """Save Index to file"""
        try:
            js = json.loads(self.reset_index().to_json(orient = 'records', date_format = 'iso'))
...
class Operator(object): #TODO: Extend stringprocessor directly?
    ...
    def fuzzy(self, scorer = None, score_cutoff = 90, limit = 1) -> pd.DataFrame:
        ...
        # result = pd.Series(name = target)
        extracted: list = process.extractBests(self.norm,
    ...
    def ratio(self) -> pd.Series:
        result: pd.Series =  self.fuzzy(scorer = fuzz.ratio,
    ...
    def token_set(self) -> pd.Series:
        result: pd.Series =  self.fuzzy(scorer = fuzz.token_set_ratio,
                                  score_cutoff = self._min_score,
                                  limit = 1)
    ...
    def token_sort(self) -> pd.Series:
        result: pd.Series = self.fuzzy(scorer = fuzz.token_sort_ratio,
                                  score_cutoff = self._min_score,
                                  limit = 1)
    ...
    def find(self) -> str:
        ...
        try:
            ...
            if alias is None:
                logger.warning(f"Unable to find alias for {self.name}. Defaulting to operator's original name.")
                alias = self.name
        except Exception as e:
            logger.warning(f"Error searching for alias for {self.name}. Defaulting to operator's original name. -- {e}")
            alias = self.name
